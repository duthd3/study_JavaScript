...
- 두번째 매개변수에는 생성할 객체의 프로퍼티 키와 프로퍼티 디스크립터 객체로 이뤄진 객체를 전달한다.(선택)
...
console.log(Object.getPrototypeOf(obj_ === myProto); //true
```
## 19.12 정적 프로퍼티/메서드
...
key in Object
```
- in연산자는 확인 대상 객체의 프로퍼티뿐만 아니라 확인 대상 객체가 상속받은 모든 프로토타입의 프로퍼티를 확인한다.
- 동일한 메서드로 Reflect.has 메서드가 있다.
```js
