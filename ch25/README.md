...
- 기존 클래스를 상속받아 새로운 클래스를 확장하여 정의 하는 것이다.
...
- `super 참조`
  ...
  - [[HomeObject]]를 가지는 함수만이 super를 참조할 수 있다. ES6의 메서드 축약표현으로 정의된 함수만이 [[HomeObject]]를 갖는다.
...
- 2.수퍼클래스의 인스턴스 생성과 this 바인딩
  - 수퍼클래스의 cocnstructor 내부의 코드가 실행되기 이전에 암묵적으로 빈 객체를 생성.
...
- 4.서브클래스 constructor로의 복귀와 this 바인딩
  ...
  - 서브클래스의 constructor에서 super를 호출가히 전에는 this를 참조할 수 없는 이유가 바로 이 때문이다.
  - 따라서 서브클래스 constructor 내부의 인스턴스 초기화는 반드시 super 호출 이후에 처리되어야 한다.
- 5.서브클래스의 인스턴스 초기화
  - super호출 이후, 서브클래스의 constructor가 인스턴스 초기화를 실행한다.
- 6.인스턴스 반환
